...
export function SessionGroupHeader({ label, count }: { label: string; count: number }) {
  return (
    <div className={`sticky top-0 z-1 -mx-2.5 flex items-center gap-1.5 bg-neutral-950/95 px-3.5 pb-1 pt-2 font-mono text-[9.5px] font-medium uppercase tracking-[0.12em] backdrop-blur-xs ${TONE[label] ?? 'text-neutral-500'}`}>
      ...
      <span className="h-px min-w-3 flex-1 bg-linear-to-r from-neutral-800 to-transparent" />
    </div>
  );
}
