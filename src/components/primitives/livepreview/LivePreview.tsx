...
// Renderiza código do assistente como TELA rodando dentro de um iframe sandbox
// (origem opaca — não alcança o app pai). `preview` = React/TSX; `preview-html`
// = HTML cru; `preview-native` = componente react-native (roda via
// react-native-web numa moldura de iPhone, sem macOS). Toggle Tela⇄Código +
// input de refino que devolve o pedido pro chat.
export function LivePreview({ code, lang }: { code: string; lang: string }) {
  ...
  const payload = useMemo((): Msg | { error: string } => {
    if (mode === 'html') return { type: 'deck:html', html: code };
  ...
  useEffect(() => {
    ...
    const onMsg = (e: MessageEvent) => {
      ...
      else if (e.data.type === 'deck:height' && mode !== 'native') setHeight(Math.min(640, Math.max(120, e.data.height + 4)));
    };
  ...
  const frame = <iframe ref={ref} title="live preview" sandbox="allow-scripts"
    ...
    <div className="my-1 overflow-hidden rounded-lg border border-orange-500/25 bg-[#0c0c0c]">
      ...
      {tab === 'code' ? (
        ...
        <div className="relative">
          {errorOverlay}
          <PhoneFrame>
            <div className="h-full w-full" style={{ width: PHONE_SCREEN.width, height: PHONE_SCREEN.height }}>
              {/* iframe preenche a tela do telefone */}
              <div className="h-full w-full [&>iframe]:block [&>iframe]:h-full [&>iframe]:w-full [&>iframe]:border-0">{frame}</div>
            </div>
          </PhoneFrame>
        </div>
      ) : (
