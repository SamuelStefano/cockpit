...
function HighlightedCode({ code, lang }: CodeBlockProps) {
  ...
  return (
    <div className="group/code my-1 rounded-lg border border-neutral-800 bg-[#0c0c0c]">
      ...
      {/* Focusable when not wrapping (lines may run past the edge): otherwise a
          keyboard user cannot reach them. A named focus stop needs a role. */}
      <pre tabIndex={wrap ? undefined : 0} role={wrap ? undefined : 'region'} aria-label={wrap ? undefined : 'código'} className={`scroll-thin overflow-x-auto px-3 py-2.5 text-[12.5px] leading-relaxed ${wrap ? 'whitespace-pre-wrap wrap-break-word' : 'whitespace-pre'}`}>
