...
export function QueuedItem({ index, text, atts, expanded, flash, first, last, editing, draft, setDraft, onToggle, onStartEdit, onCommit, onCancelEdit, onMove, onRemove }: Props) {
  const taRef = useRef<HTMLTextAreaElement>(null);
  useEffect(() => { if (editing) taRef.current?.focus(); }, [editing]);
  return (
    <li className={`flex items-start gap-1 rounded-md transition-colors duration-500 ${flash ? 'bg-orange-500/20' : ''}`}>
