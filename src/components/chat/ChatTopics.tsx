...
export function ChatTopics({ topics, activeId, open, setOpen, onJump }: ChatTopicsProps) {
  ...
  return (
    ...
    >
      ...
      {open && (
        <nav aria-label="Tópicos da conversa" className={`fade-up pointer-events-auto absolute right-full top-1/2 mr-1 flex max-h-full w-60 max-w-[70vw] -translate-y-1/2 flex-col overflow-hidden ${tokens.radius.lg} ${tokens.surface.raised} py-1.5`}>
