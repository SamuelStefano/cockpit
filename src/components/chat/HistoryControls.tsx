...
// In a header narrower than 36rem the label does not fit next to the title (A6,
// and the 1024px desktop chat column): square icon-only button, text in
// `aria-label`/`title`. Container query — ChatHeader is the @container.
const base = `flex h-7 w-7 items-center justify-center gap-1 whitespace-nowrap rounded-md border text-[10.5px] transition @min-[36rem]:h-auto @min-[36rem]:w-auto @min-[36rem]:px-1.5 @min-[36rem]:py-0.5 ${tokens.focusRing} ${tokens.touchTarget}`;
const label = 'hidden @min-[36rem]:inline';
