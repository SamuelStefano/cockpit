    ...
    <div className="pointer-events-auto absolute left-3 top-3 flex max-h-[calc(100%-1.5rem)] w-64 flex-col rounded-lg border border-neutral-800 bg-neutral-900/95 shadow-xl backdrop-blur-sm">
      <div className="flex items-start gap-2 border-b border-neutral-800/70 p-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="wrap-break-word font-mono text-[13px] text-neutral-100">{node.label}</div>
          ...
          <div className="mt-1.5 flex flex-wrap items-center gap-1">
            {node.repo && <span className="rounded-sm bg-neutral-800 px-1.5 py-0.5 font-mono text-[10px] text-orange-300">{node.repo}</span>}
            <span className="rounded-sm bg-neutral-800 px-1.5 py-0.5 font-mono text-[10px] text-neutral-400">{node.deg} conexões</span>
          </div>
        </div>
        <button onClick={onClose} aria-label="Fechar detalhe" className="shrink-0 rounded-sm p-0.5 text-neutral-500 hover:text-neutral-200">
          <Icon name="x" size={14} />
        </button>
      </div>
            ...
            {neighbors.map(({ node: n, relation }) => (
              <li key={n.id}>
                <button
                  ...
                  className="flex w-full items-center gap-2 rounded-sm px-1.5 py-1 text-left hover:bg-neutral-800/70"
                >
