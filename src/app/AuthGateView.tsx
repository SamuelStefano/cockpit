...
// sessão, nada do app aparece. Recuperação de senha vem ANTES de tudo — o link do
// e-mail já traz sessão válida, então sem esta trava o app abriria com a senha
// antiga ainda valendo. Logado mas a VPS ainda não atende → dashboard de
...
export function resolveAuthGate({ sbAuth, ejectPairing, authRequired, submitToken }: AuthGateViewProps) {
  ...
  if (SUPABASE_ENABLED) {
    if (sbAuth.recovery && sbAuth.session) {
      node = (
        <PasswordRecoveryGate
          changePassword={sbAuth.changePassword}
          onDone={sbAuth.endRecovery}
          onCancel={() => { void sbAuth.signOut().then(sbAuth.endRecovery); }}
        />
      );
    } else if (!sbAuth.loading && !sbAuth.session) node = <SupabaseAuthGate auth={sbAuth} />;
