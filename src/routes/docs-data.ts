...
export const SECTIONS: Section[] = [
  ...
  { id: 'afunilar', label: 'Afunilar sessões', icon: 'sparkles' },
  { id: 'tempo-real', label: 'Tempo real', icon: 'circle' },
  { id: 'contexto', label: 'Contexto & tokens', icon: 'zap' },
  { id: 'tokens', label: 'Economia de contexto', icon: 'zap' },
  ...
  { id: 'rotas', label: 'Rotas de trabalho', icon: 'grip' },
...
export const FILEMAP: { group: string; tone: string; files: { path: string; what: string }[] }[] = [
  ...
  },
  {
    ...
    files: [
      ...
      { path: 'src/components/sessions/stale.ts · FunnelModal.tsx', what: 'Afunilar: escolhe as sessões paradas (protegendo favorita/aberta/rodando) e o modal que mostra o prompt antes de rodar.' },
  ...
  },
  {
    ...
    files: [
      ...
      { path: 'server/handoff.ts', what: 'Migra UMA sessão lotada: destila um briefing em Contextos e arquiva a sessão.' },
      { path: 'server/funnel.ts', what: 'Afunilamento: destila N sessões paradas num dossiê único (funil-AAAAMMDD) e só então arquiva todas.' },
