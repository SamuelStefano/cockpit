...
export function PontosWorkspace(props: Props) {
  const { snapshot, now, syncing, onSync } = props;
  ...
  return (
    <div className="scroll-thin flex min-h-0 flex-1 flex-col overflow-y-auto lg:overflow-hidden">
      <div className="mx-auto flex w-full max-w-[1600px] flex-col lg:min-h-0 lg:flex-1 2xl:border-x 2xl:border-neutral-800/60">
        <PontosTopBar state={pg.monthState} onNewEpic={() => pg.newEpic.set(true)} />
        <KpiStrip snapshot={snapshot} totals={pg.totals} offPoints={pg.offPoints} offAmountCents={pg.offAmountCents}
          drafts={pg.draftTotals} now={now} syncing={syncing} onSync={onSync} />
        ...
        <div className="flex lg:min-h-0 lg:flex-1">
          <aside className="hidden w-[340px] shrink-0 flex-col border-r border-neutral-800/80 lg:flex xl:w-[372px]">{navigator}</aside>
