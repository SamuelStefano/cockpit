...
export const CanvasFlowArrows = memo(function CanvasFlowArrows({ nodes, pos, windows, compact, flows, firedAt, onFlowClick }: Props) {
  const now = Date.now();
      ...
      {flows.map((f) => {
        ...
        return (
          <g key={f.id}>
            ...
            />
            <g className="pointer-events-auto cursor-pointer" onClick={() => onFlowClick(f.id)}>
