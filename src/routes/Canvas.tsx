  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col bg-neutral-950">
      ...
      {!p.connected ? (
        ...
        <EmptyState icon="layers" title="Montando o grafo" description="A primeira leitura varre todos os transcripts; depois é incremental." />
      ) : (
          ...
          {showKanban && (
            <div className={`flex min-h-0 flex-col border-t border-neutral-800 ${showCanvas ? 'flex-[2]' : 'flex-1'}`}>
              <Kanban
                ...
                onSelect={(id) => (showCanvas ? focusNode(`k:${id}`) : r.editCard(id))} onMove={r.setStatus}
